...
from db import is_subscription_active, init_db
init_db()
...
def webhook():
    ...
    # Проверим, успешная ли оплата
    if payload.get("event") == "payment.succeeded":
        payment = payload["object"]
        user_id = int(payment["metadata"]["user_id"])
        sub_type = payment["metadata"].get("subscription_type", "trial")

        # Добавим подписку на 30 дней
        subscriptions[user_id] = datetime.now() + timedelta(days=30)
        print(f"✅ Подписка ({sub_type}) активирована для пользователя {user_id}")

        # Если это пробная подписка, запускаем таймер автосписания
        if sub_type == "trial":
            def auto_renew():
                print(f"🔁 Автопродление для {user_id}")
                payment = Payment.create({
                    ...
                    "confirmation": {
                        "type": "redirect",
                        "return_url": "https://t.me/TenevoeTaro_bot"
                    },
                    "capture": True,
                ...
                # Можно уведомить пользователя (если хочешь)
                bot.send_message(user_id,
                                 f"🔁 Срок пробной подписки истёк. Перейдите по ссылке для продления:\n{payment.confirmation.confirmation_url}")

            # Запустить таймер на 30 дней
            from threading import Timer
            Timer(30 * 24 * 60 * 60, auto_renew).start()
        payment = payload["object"]
        user_id = int(payment["metadata"]["user_id"])

        # Поток для Flask (обработка webhook от ЮKassa)

        if __name__ == '__main__':
            port = int(os.environ.get("PORT", 5000))
            app.run(host='0.0.0.0', port=port)

    return "", 200  # ответ обязательно 200!
...
def handle_subscriptions(message):
    ...
    now = datetime.now()
    if user_id in subscriptions and subscriptions[user_id] > now:
        bot.send_message(user_id, f"✅ Подписка активна до {subscriptions[user_id].strftime('%d.%m.%Y %H:%M')}")
    else:
        markup = types.InlineKeyboardMarkup()
